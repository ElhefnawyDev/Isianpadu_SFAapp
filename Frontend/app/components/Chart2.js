...
const BarChartComponent2 = ({year}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setTotalProjects(chartData.totalProjectsCurrentYear);
        setTotalAmount(chartData.totalAmountCurrentYear);
      } catch (error) {
  ...
  const barData = data.map((item) => ({
    value: item.amount / 1000000, // Converting to millions for display
    label: item.month,
  ...
  return (
    <View>
      ...
      <View style={styles.detailRow}>
          ...
          <Text style={styles.valueText}>{selectedBar?.month || "N/A"}</Text>
        </View>
